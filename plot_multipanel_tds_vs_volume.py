...
df_time_dependent["mass__g"] = df_time_dependent["mass_flow_rate__g_per_s"].cumsum() * dt
df_time_dependent["solids__g"] = df_time_dependent["solids_flow__g_per_s"].cumsum() * dt

ax_volume.plot(
    df_time_dependent["mass__g"],
    df_time_dependent["mass_flow_rate__g_per_s"]
)

ax_solutes.plot(
    df_time_dependent["solids__g"],
    df_time_dependent["mass_flow_rate__g_per_s"]
)
...
# Axis formatting
# =========================
ax_volume.set_xlabel("Mass in cup [g]")
ax_volume.set_ylabel("Flow rate [g/s]")
ax_volume.set_ylim(top=2.2)

ax_solutes.set_xlabel("Disolved solids [g]")
ax_solutes.set_ylabel("Flow rate [g/s]")
ax_solutes.set_ylim(top=2.2)
...
out_file = OUTPUT_DIR / "multipanel_tds_vs_volume.pdf"
